...
{
	function StatisticsPage ( $CurrentUser )
	{
		global $lang;
		...
		else
		{
			...
			while ($StatRow = mysql_fetch_assoc($query))
			{
				...
				if ($StatRow['id'] != $previusId && $StatRow['id_owner'] != 1 && $StatRow['id_owner'] != 2)  // Jonamix id 1  // Lugii id 2
				{
					$previusId 			= $StatRow['id'];
					$parse['ranking_data'] = "".date ("Y-m-d H:i:s", $StatRow['id'])."";
