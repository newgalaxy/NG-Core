...
class InfosPage
{
	 ...
	 */
	function ShowStorageTable ($CurrentUser, $CurrentPlanet, $BuildID, $Template)
    {
        global $resource;
		
			$CurrentBuildtLvl = $CurrentPlanet[ $resource[$BuildID] ];
			$BuildStartLvl    = max(0,$CurrentBuildtLvl - 2);
			$Table     = "";
			$ProdFirst = 0;
			$ActualProd = Production::max_storable($BuildLevel);
		for ( $BuildLevel = $BuildStartLvl; $BuildLevel < $BuildStartLvl + 15; ++$BuildLevel ) 
		{
			$Prod = Production::max_storable($BuildLevel);
			$bloc['build_lvl']  = ($CurrentBuildtLvl == $BuildLevel) ? "<font color=\"#ff0000\">".$BuildLevel."</font>" : $BuildLevel;
			if ($ProdFirst > 0) 
			{
				$bloc['build_gain']      = "<font color=\"lime\">(". Format::shortly_number(floor($Prod - $ProdFirst)) .")</font>";
			} 
			else 
			{
				$bloc['build_gain']      = "";
			}
			$bloc['build_prod']      = Format::shortly_number(floor($Prod));
			$bloc['build_prod_diff'] = Format::color_number( Format::shortly_number(floor($Prod - $ActualProd)) );
			if ($ProdFirst == 0) 
			{
	 ...
	 */
    function astrophysics_table ($CurrentUser, $CurrentPlanet, $BuildID, $Template)
    {
		$CurrentBuildtLvl = $CurrentPlanet[ $resource[$BuildID] ];
		...
		for ( $BuildLevel = $BuildStartLvl ; $BuildLevel < $BuildStartLvl + 15 ; ++$BuildLevel )
		{
			...
			$bloc['tech_colonies']      = Format::pretty_number ( Fleets::get_max_colonies ( $BuildLevel ) );
			$bloc['tech_expeditions']	= Format::pretty_number ( Fleets::get_max_expeditions ( $BuildLevel ) );
    ...
    public function __construct ($CurrentUser, $CurrentPlanet, $BuildID)
    {
        ...
        {
            $PageTPL              = gettemplate('infos/info_buildings_table');
            $TableHeadTPL         = gettemplate('infos/info_storeages_header');
            $TableTPL             = gettemplate('infos/info_storeages_body');
        }
        elseif ($BuildID ==  42)
        {
		...
		elseif ( $BuildID ==  124 )
        {
            $PageTPL              = gettemplate ( 'infos/info_buildings_table' );
            $DestroyTPL           = gettemplate ( 'infos/info_buildings_destroy' );
            $TableHeadTPL         = gettemplate ( 'infos/info_astrophysics_header' );
            $TableTPL             = gettemplate ( 'infos/info_astrophysics_table' );
            $TableFooterTPL       = gettemplate ( 'infos/info_astrophysics_footer' );
        }
        ...
        if($BuildID >= 22 && $BuildID <= 24){
            $parse['table_data']  = $this->ShowStorageTable ($CurrentUser, $CurrentPlanet, $BuildID, $TableTPL);
        }elseif ( $BuildID == 124 ){
			$parse['table_data']	= $this->astrophysics_table ($CurrentUser, $CurrentPlanet, $BuildID, $TableTPL);
		}
		else{
		...
		if ( $TableFooterTPL != '' )
		{
			$parse['table_footer']  	= parsetemplate  ( $TableFooterTPL , $lang );
		}
