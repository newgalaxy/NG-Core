	...
	public function __construct($CurrentUser)
	{
		...
		if ($_POST && $mode == "change")
		{
			...
			if ($CurrentUser['username'] != $_POST["db_character"])
			{
				...
				if (!$query)
                {
					if ($CurrentUser['uctime'] == "0" && $CurrentUser['uctime'] < "time()")
					{
						$zaman  = time();
						$uctime = time() + 1296000;//Tiempo limete 604800 segundos son 7 dias
						
						doquery("UPDATE {{table}} SET uctime='".$uctime."' WHERE id='".$CurrentUser['id']."' LIMIT 1", "users");
						
						if ($CurrentUser["uctime"] < "time()" && $CurrentUser["uctime"] != "0")
						{
							doquery("UPDATE {{table}} SET uctime='0' WHERE id='".$CurrentUser['id']."' LIMIT 1", "users");
						}
					...
					{
						message($lang['op_change_nick_exit'], "game.php?page=options", 2); 
					}
				}
			}
		...
		else
		{
			...
			else
			{
				...
				$SkinsFolder = opendir(XGP_ROOT . 'styles/themes');
