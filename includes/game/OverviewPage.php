	...
	function __construct ( $CurrentUser , $CurrentPlanet )
	{
		...
		switch($_GET['mode'])
		{
			...
			default:
				...
				{
					$messages_link = ($CurrentUser['rpg_commandant'] > 0) ?"":"&mode=show&messcat=100";
					$Have_new_message .= "<tr>";
					if($CurrentUser['new_message'] == 1)
					{
						$Have_new_message .= "<th colspan=4><a href=game.php?page=messages".$messages_link.">" . $lang['ov_have_new_message'] . "</a></th>";
					}
					elseif($CurrentUser['new_message'] > 1)
					{
						$Have_new_message .= "<th colspan=4><a href=game.php?page=messages".$messages_link.">";
				...
				if ($CurrentUser['rpg_commandant'] > 0){
					$parse['opt_commandant']  	= "";
				}
				else{
					$parse['opt_commandant']  	= parsetemplate(gettemplate('overview/overview_body_extra'), $parse);
				}
