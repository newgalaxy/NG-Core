				...
				if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element))
				{
					...
					if ($CanBuildOne && $NotBuilding)
					{
						...
						if ($CurrentUser['rpg_commandant'] > 0)
							$parse['max']       = "<a href='javascript:' onclick=\"document.getElementsByName('fmenge[".$Element."]')[0].value = '$maxElement';\">(Max. ".$maxElement.")</a>"; 
					}
				...
				if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element))
				{
					...
					if ($CanBuildOne)
					{
						...
						elseif($NotBuilding)
						{
							...
							$parse['add_element'] = "<input type=text name=fmenge[".$Element."] alt='".$lang['tech'][$Element]."' size=6 maxlength=6 value=0 tabindex=".$TabIndex.">";
							if ($CurrentUser['rpg_commandant'] > 0)
								$parse['max']           = ($max)?"<br><a href='javascript:' onclick=\"document.getElementsByName('fmenge[".$Element."]')[0].value = '$max';\">(Max. ".$max.")</a>":"<br><a href='javascript:' onclick=\"document.getElementsByName('fmenge[".$Element."]')[0].value = '$maxElement';\">(Max. ".$maxElement.")</a>"; 
						}
