...
class GalaxyRows
{
		...
		if ($GalaxyInfo)
		{
			if ($GalaxyInfo["metal"]+$GalaxyInfo["crystal"] >= DEBRIS_MIN_VISIBLE_SIZE)
			{
				...
				if ($user['rpg_commandant'] > 0){
					$parse['opt_commandant']  	= "<tr><td class=c colspan=2>".$lang['gl_actions'].":</td></tr><tr><th colspan=2 align=left><a href= # onclick=&#039javascript:doit (8, {galaxy}, {system}, {planet}, {planettype}, {recsended}); return nd();&#039 >".$lang['gl_collect']."</a></tr>";
				}
				else{
	...
	public function GalaxyRowMoon($GalaxyInfo, $Galaxy, $System, $Planet, $PlanetType)
	{
		...
		if ($GalaxyInfo['id'] != $user['id'] && $user['rpg_commandant'] > 0)
	...
	public function GalaxyRowUser($GalaxyInfo, $Galaxy, $System, $Planet)
	{
		...
		if ($GalaxyInfo && $GalaxyInfo["destruyed"] == 0)
		{
			...
			if ($GalaxyInfo['authlevel'] == 3) {
				$admin = "(<font color=\"orange\">A</font>)";
			}
